...
Для опытных, две опции.

**Свой nginx.** Нужен nginx со stream-модулем (`ngx_stream_module`; в стоковом пакете обычно есть, иначе `load_module`). Свободные порты **443** (прошивки/иконки) и **8883** (MQTT); в `resolver` — рабочий DNS. Кладёте `stream{}`-блоки из примеров выше, `nginx -t && nginx -s reload`. Боевой сервер поднят так.

**Docker Compose.** Образ и конфиги собраны вместе:

```bash
docker compose up -d   # spinogrizz/unifi-proxy:latest, порты 443 + 8883
```

Порты 443 и 8883 на хосте должны быть свободны. Из коробки работают SNI-проксирование (отдаёт настоящий серт Ubiquiti), MQTT-passthrough и whitelist (не-ubnt SNI → reset).
